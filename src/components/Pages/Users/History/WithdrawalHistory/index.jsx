...
const WithdrawalHistory = () => {
  ...
  const { withdrawalHistory } = useSelector(historySelector);

  const withdrawalHistoryLastPage = withdrawalHistory?.last_page;
  ...
  useEffect(() => {
    dispatch({
      type: types.GET_WITHDRAWAL_HISTORY_START,
      payload: {
        params: {
          currency_type: 'all',
  ...
  useEffect(() => {
    if (withdrawalHistory && withdrawalHistory?.data?.length) {
      setData(prev => [...prev, ...withdrawalHistory?.data]);
    }
  }, [withdrawalHistory]);

  const handleNextData = () => {
    if (withdrawalHistoryLastPage) {
      if (currentPage === withdrawalHistoryLastPage) {
    ...
    <div className="profile-box">
      <div className="table-panel">
        <p className="item-title">
          {L.translate('Pages.Users.History.WithdrawalHistory.item1')}
        </p>
          ...
          <div className="filter-search">
            <div className="filter-search__reset">
              <button
                ...
                onClick={handleReset}
              >
                {L.translate('Pages.Users.History.WithdrawalHistory.item2')}
              </button>
            </div>
              ...
              <div className="field-wrap">
                <input
                  ...
                  placeholder={L.translate(
                    'Pages.Users.History.WithdrawalHistory.item4',
                  )}
                  value={searchPhrase}
      ...
      <div className="profile-table">
        <div
          ...
          id="withdrawal-history-component-table-box"
        >
          <div className="table table--auto-td-height table--history-deposit">
            ...
            <div className="table-body">
              <InfiniteScrollList
                ...
                parentRef="withdrawal-history-component-table-box"
              >
                {historyList && historyList?.length ? (
                  ...
                  <EmptyTable
                    text={L.translate(
                      'Pages.Users.History.WithdrawalHistory.item3',
                    )}
                  />
                )}
              </InfiniteScrollList>
