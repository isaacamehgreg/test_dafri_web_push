...
import { swapSpotSelector } from '../../../../../redux/trade/spot/selectors';
...
const SwapHistory = () => {
  ...
  const orders = useSelector(swapSpotSelector);
  ...
  const SwapHistory = findAnyValueInObject(data, searchPhrase, [
    'amount',
    'created_at',
    'to_asset_code',
    'quantity',
        ...
        <div className="tr" key={item.id}>
          <div className="td">
            <div className="td-hidden-name">
              {L.translate('Pages.Users.History.OpenOrders.item34')}
            </div>
            <p>{transformData(item?.created_at)}</p>
            <p>{console.log('ddddd', item.created_at)}</p>
          </div>
          <div className="td">
            <div className="td-hidden-name">
              {L.translate('Pages.Users.History.OpenOrders.item33')}
            </div>
            <p>{item?.from_asset_code.toUpperCase()}</p>
          </div>
          <div className="td">
            <div className="td-hidden-name">
              {L.translate('Pages.Users.History.OpenOrders.item33')}
            </div>
            <p>{item?.to_asset_code.toUpperCase()}</p>
          </div>
          <div className="td">
            <div className="td-hidden-name">
              {L.translate('Pages.Users.History.OpenOrders.item27')}
            </div>
            <div className="table-coin">
              <p className="table-coin__name">{item?.exchange_amount}</p>
            </div>
          </div>
          <div className="td">
            <div className="td-hidden-name">
              {L.translate('Pages.Users.History.OpenOrders.item32')}
            </div>
            <p>{item?.quantity}</p>
          </div>
          <div className="td">
            <div className="td-hidden-name">
              {L.translate('Pages.Users.History.OpenOrders.item29')}
            </div>
            <p>{item?.exchange_fee}</p>
          </div>
          <div className="td">
            <div className="td-hidden-name">
              {L.translate('Pages.Users.History.OpenOrders.item28')}
            </div>
  ...
  useEffect(() => {
    dispatch({
      type: types.Swap_GET_ORDERS_LIST_START,
      payload: {
        params: {
          per_page: 5,
    ...
    <div className="profile-box">
      <div className="table-panel">
        <p className="item-title">
          {L.translate('Pages.Users.History.OpenOrders.item26')}
        </p>
      ...
      <div className="profile-table">
        ...
        >
          <div className="table table--auto-td-height table--open-orders-sl newtab">
            <div className="table-header">
              <div className="tr">
                ...
                <div className="td">
                  <div className="td-name">
                    {L.translate('Pages.Users.History.OpenOrders.item31')}
                  </div>
                </div>
                <div className="td">
                  <div className="td-name">
            ...
            <div className="table-body">
              ...
              >
                {SwapHistory.length ? (
                  ...
                  <EmptyTable
                    text={L.translate('Pages.Users.History.OpenOrders.item35')}
                  />
                )}
              </InfiniteScrollList>
            </div>
