...
import {
  instanceExchangeSelector,
  secondExchangeAssetsSelector,
...
const TransferTab = ({ type = 'swap' }) => {
  ...
  const { rate, rateExchange } = useSelector(instanceExchangeSelector);
  ...
  const handleChange = e => {
    if (e.target.name === 'firstAsset' && numberValidation(e.target.value)) {
      setFirstAssetQuantity(e.target.value);
      setSecondAssetQuantity(e.target.value * rate);
    }

    if (e.target.name === 'secondAsset' && numberValidation(e.target.value)) {
      setSecondAssetQuantity(e.target.value);
      setFirstAssetQuantity(e.target.value / rate);
    }
  };
  ...
  const handleUpdateRate = () => {
    if (firstAsset && secondAsset) {
      dispatch({
        ...
        payload: {
          from_asset_id: firstAsset.id,
          to_asset_id: secondAsset.id,
  ...
  useEffect(() => {
    if (availableAssets) {
      const assetsFromArr = Object.values(availableAssets)?.map(
        obj => obj.asset_from,
      );

      const assetsToArrAll = Object.values(availableAssets);
      const assetsToArr = Object.values(
        assetsToArrAll.filter(
          ({ asset_from, asset_to }) => asset_from.id === assetsFromArr[0].id,
        ),
      );

      setAssetsFrom(assetsFromArr);
      setAssetsTo(assetsToArr[0].asset_to);

      setFirstAsset(assetsFromArr[0]);
      setSecondAsset(assetsToArr[0].asset_to[0]);
    }
  }, [availableAssets]);

  useEffect(() => {
    if (availableAssets && firstAsset) {
      setAssetsTo(availableAssets[firstAsset.id]?.asset_to);
      setSecondAsset(availableAssets[firstAsset.id]?.asset_to[0]);
    }
  }, [firstAsset]);

  useEffect(() => {
    setSecondAssetQuantity(rateExchange?.before_fee || '0');
  }, [rateExchange, rate]);
    ...
    if (captcha) {
      dispatch({
        ...
        payload: {
          ...
          to_asset_id: secondAsset.id,
          quantity: firstAssetQuantity,
          type: type === 'swap' ? 'user' : 'payment_agent',
          captcha,
        },
      });

      setFirstAssetQuantity('0');
      setSecondAssetQuantity('0');
    }
  }, [captcha]);
  ...
  return (
    <div className="transfer-block">
      <p className="item-title item-title--bigger my-balance__title">
        {L.translate('Pages.Users.Transfer.PaymentAgent.item1')}
      </p>

      <div className="methods-row methods-row--transfer">
        <Dropdown
          assets={assetsFrom}
          selectedCoin={firstAsset || assetsFrom[0]}
          onSelectCurrency={setFirstAsset}
          label={L.translate('Pages.Users.Transfer.PaymentAgent.item2')}
        />

        <Dropdown
          assets={assetsTo}
          selectedCoin={secondAsset || assetsTo[0]}
          onSelectCurrency={setSecondAsset}
          label={L.translate('Pages.Users.Transfer.PaymentAgent.item3')}
        />
      </div>

      <div className="methods-row methods-row--transfer methods-row--mt">
        <TransferField
          label={L.translate('Pages.Users.Transfer.PaymentAgent.item4')}
          selectedCoin={firstAsset}
          onChange={handleChange}
          quantity={firstAssetQuantity}
          name="firstAsset"
        />

        {/* <div className="methods-row__icon">
          <svg
            width="26"
            height="26"
            viewBox="0 0 26 26"
        ...
        <TransferField
          label={L.translate('Pages.Users.Transfer.PaymentAgent.item5')}
          selectedCoin={secondAsset}
          onChange={handleChange}
          quantity={secondAssetQuantity || rate}
          name="secondAsset"
        />
      </div>

      <TransferBox
        firstQuantity={firstAssetQuantity}
        secondQuantity={secondAssetQuantity}
